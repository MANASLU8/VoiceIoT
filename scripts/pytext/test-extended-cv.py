...
def parse_lines(lines):
    for command in data:
        text = command.split('\t')[-3]
        slots = [slot.split(':') for slot in command.split('\t')[1].split(',')]
        ...
        yield {"command": text, "slots": enriched_slots, "device": command.split('\t')[0], "original": command}
...
for i in range(len(sets)):
    ...
    os.system('pytext train < vendor/pytext/bilstm-local.json')
    ...
    for label in test_dataset.keys():
        ...
        for sample in test_dataset[label]:
            true_label = sample['device']
            recognized = [slot for slot in get_best_slots(predictor(
                {"text": sample["command"].lower(), "doc_weight": 1, "word_weight": 1}))]  # if slot != '__UNKNOWN__']
            parsed_command = list(zip(sample['command'].lower().split(' '), recognized))
            parsed_right_command = list(zip(sample['command'].lower().split(' '), sample['slots']))
            ...
            output_commands.append({"command": list(zip(sample['command'].split(' '), recognized)), "device": sample['device']})
            ...
            recognized_label = get_best_label(
                predictor({"text": sample['command'].lower(), "doc_weight": 1, "word_weight": 1}))
            ...
            print(f"{sample['command']}\t{label}\t{ontology_label}\t{ontology_label_score}\t{ontology_unmapped_label}\t{raw_ontology_label}\t{raw_ontology_label_score}\t{raw_ontology_unmapped_label}\t{recognized_label}\t{labels['labels'][1] if len(labels['labels']) > 1 else ''}\t{' '.join(raw_ontology_label_text)}\t{ontology_label_text}")
...
fo.write_json("labelled-commands-pytext.json", output_commands)
