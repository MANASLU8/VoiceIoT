...
for sample in test_dataset:
    ...
    true_label = label#sample['intent']
    sample_text = ' '.join([item[0] for item in sample["command"]])
    recognized = [slot for slot in get_best_slots(predictor(
        {"text": sample_text.lower(), "doc_weight": 1, "word_weight": 1}))]  # if slot != '__UNKNOWN__']
    parsed_command = list(zip(sample_text.lower().split(' '), recognized))
    parsed_right_command = sample['command']#list(zip(sample['text'].lower().split(' '), sample['slots']))
    ...
    recognized_label = get_best_label(
        predictor({"text": sample_text.lower(), "doc_weight": 1, "word_weight": 1}))
    ...
    print(f"{sample_text}\t{label}\t{ontology_label}\t{ontology_label_score}\t{ontology_unmapped_label}\t{raw_ontology_label}\t{raw_ontology_label_score}\t{raw_ontology_unmapped_label}\t{recognized_label}\t{labels['labels'][1] if len(labels['labels']) > 1 else ''}\t{' '.join(raw_ontology_label_text)}\t{ontology_label_text}")
